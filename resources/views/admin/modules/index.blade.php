                    ...
                    <table class="min-w-full divide-y divide-white/5">
                        ...
                        <tbody class="text-sm divide-y divide-white/5 text-gray-300">
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors">
                                <td class="py-4 font-semibold text-white">{{ $module->name }}</td>
                                <td class="py-4">
                                    <span class="px-2.5 py-1 rounded-full text-xs font-bold bg-[#8b5cf6]/10 border border-[#8b5cf6]/20 text-[#8b5cf6]">{{ $module->department->name }}</span>
                                </td>
                                <td class="py-4">
                                    <div class="flex flex-wrap gap-1">
                                        @foreach($module->professors as $professor)
                                            <span class="px-2 py-0.5 rounded-full text-xs font-bold bg-[#06b6d4]/10 border border-[#06b6d4]/20 text-[#06b6d4]">
                                                {{ $professor->user->name }}
                                            </span>
                                        @endforeach
                                    </div>
