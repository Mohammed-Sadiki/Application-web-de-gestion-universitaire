<x-app-layout>
    <x-slot name="header">
        <div class="topbar-title">Modifier le Module</div>
        <div class="topbar-subtitle">Mettre à jour les informations de {{ $module->name }}</div>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('admin.modules.update', $module) }}">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input id="name" type="text" name="name" value="{{ old('name', $module->name) }}" required autofocus
                            ...
                            <div>
                                ...
                                <select id="department_id" name="department_id"
                                    ...
                                    @foreach($departments as $department)
                                        <option value="{{ $department->id }}" {{ $module->department_id == $department->id ? 'selected' : '' }}>{{ $department->name }}</option>
                                    @endforeach
                                </select>
                                    ...
                                    @foreach($professors as $professor)
                                        <label class="flex items-center gap-2 cursor-pointer group">
                                            <input type="checkbox" name="professors[]" value="{{ $professor->id }}"
                                                {{ $module->professors->contains($professor->id) ? 'checked' : '' }}
                                                class="rounded border-white/20 bg-white/10 text-[#8b5cf6] focus:ring-[#8b5cf6]">
                                            <span class="text-sm text-gray-300 group-hover:text-white transition">{{ $professor->user->name }}</span>
                                        </label>
                                    @endforeach
                                </div>
                                @error('professors')<p class="text-[#d946ef] text-xs mt-1">{{ $message }}</p>@enderror
                            </div>
                        </div>
