<x-app-layout>
    <x-slot name="header">
        <div class="topbar-title">Modifier le Groupe</div>
        <div class="topbar-subtitle">Mettre à jour les informations de {{ $group->name }}</div>
    </x-slot>
            ...
            <div class="dark-card rounded-3xl overflow-hidden border border-white/5">
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.groups.update', $group) }}">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label for="name" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Nom du groupe</label>
                            ...
                            <div>
                                ...
                                <select id="department_id" name="department_id"
                                    ...
                                    @foreach($departments as $department)
                                        <option value="{{ $department->id }}" {{ $group->department_id == $department->id ? 'selected' : '' }}>{{ $department->name }}</option>
                                    @endforeach
                                </select>
