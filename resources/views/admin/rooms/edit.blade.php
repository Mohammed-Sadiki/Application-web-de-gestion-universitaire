<x-app-layout>
    <x-slot name="header">
        <div class="topbar-title">Modifier la Salle</div>
        <div class="topbar-subtitle">Mettre à jour les informations de {{ $room->name }}</div>
    </x-slot>
            ...
            <div class="dark-card rounded-3xl overflow-hidden border border-white/5">
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.rooms.update', $room) }}">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label for="name" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Nom de la salle</label>
                            ...
                            <div>
                                <label for="capacity" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Capacité (places)</label>
                                <input id="capacity" type="number" name="capacity" value="{{ old('capacity', $room->capacity) }}" required min="1"
                                    class="w-full bg-white/5 border border-white/10 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#8b5cf6] transition">
                            ...
                            <div>
                                ...
                                <select id="type" name="type"
                                    ...
                                    <option value="Labo" {{ $room->type === 'Labo' ? 'selected' : '' }}>Labo</option>
                                </select>
