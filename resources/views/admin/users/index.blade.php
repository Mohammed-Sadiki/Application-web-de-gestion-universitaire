<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            <div>
                <div class="topbar-title">{{ __('app.users') }}</div>
                <div class="topbar-subtitle">{{ __('app.users_list') }}</div>
            </div>
            <a href="{{ route('admin.users.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#8b5cf6] to-[#d946ef] hover:opacity-90 text-white text-xs font-bold rounded-xl shadow-md transition duration-200">
    ...
    <div class="py-6 animate-fade-in">
        ...
        <div class="dark-card rounded-3xl overflow-hidden border border-white/5">
            ...
            <div class="p-6">
                @if($users->isEmpty())
                    <p class="text-gray-400 italic text-center py-6">{{ __('app.no_pending_requests') }}</p>
                @else
                    ...
                    <table class="min-w-full divide-y divide-white/5">
                        <thead>
                            <tr class="text-left text-xs font-bold text-gray-400 uppercase tracking-wider">
                                ...
                                <th class="pb-3">{{ __('app.email') }}</th>
                                <th class="pb-3">{{ __('app.role') }}</th>
                        ...
                        <tbody class="text-sm divide-y divide-white/5 text-gray-300">
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors">
                                <td class="py-4 font-semibold text-white">{{ $user->name }}</td>
                                <td class="py-4">{{ $user->email }}</td>
                                <td class="py-4">
                                    <span class="px-2.5 py-1 rounded-full text-xs font-bold
                                        {{ $user->role === 'admin' ? 'bg-[#d946ef]/10 border border-[#d946ef]/20 text-[#d946ef]' : ($user->role === 'professor' ? 'bg-[#06b6d4]/10 border border-[#06b6d4]/20 text-[#06b6d4]' : 'bg-[#8b5cf6]/10 border border-[#8b5cf6]/20 text-[#8b5cf6]') }}">
                                        {{ ucfirst($user->role) }}
                                    </span>
                                </td>
                                <td class="py-4 text-right space-x-2 whitespace-nowrap">
                                    <a href="{{ route('admin.users.edit', $user) }}" class="inline-flex items-center px-3 py-1 bg-[#8b5cf6]/10 hover:bg-[#8b5cf6]/20 border border-[#8b5cf6]/20 text-[#8b5cf6] text-xs font-bold rounded-xl transition">{{ __('app.edit') }}</a>
