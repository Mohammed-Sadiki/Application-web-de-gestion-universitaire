<x-app-layout>
    <x-slot name="header">
        <div class="topbar-title">{{ __('app.edit_user') }}</div>
            ...
            <div class="dark-card rounded-3xl overflow-hidden border border-white/5">
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.users.update', $user) }}">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label for="name" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">{{ __('app.name') }}</label>
                            ...
                            <div>
                                <label for="email" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">{{ __('app.email') }}</label>
                            ...
                            <div>
                                <label for="role" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">{{ __('app.role') }}</label>
                                <select id="role" name="role"
                                    ...
                                    <option value="admin" {{ $user->role === 'admin' ? 'selected' : '' }}>{{ __('app.admins') }}</option>
                                    <option value="professor" {{ $user->role === 'professor' ? 'selected' : '' }}>{{ __('app.professor_section') }}</option>
                                    <option value="student" {{ $user->role === 'student' ? 'selected' : '' }}>{{ __('app.student_section') }}</option>
                                </select>
