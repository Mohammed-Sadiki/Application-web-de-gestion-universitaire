<x-app-layout>
    <x-slot name="header">
        <div class="topbar-title">{{ __('app.edit_reservation') }}</div>
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.reservations.update', $reservation) }}">
                        ...
                        <div class="space-y-4">
                                ...
                                <select id="room_id" name="room_id" required
                                    ...
                                    @foreach($rooms as $room)
                                        <option value="{{ $room->id }}" {{ old('room_id', $reservation->room_id) == $room->id ? 'selected' : '' }}>
                                            {{ $room->name }} ({{ __('app.places', ['count' => $room->capacity]) }})
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                <label for="date" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">{{ __('app.date') }}</label>
                            ...
                            <div>
                                <label for="reason" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">{{ __('app.reason') }}</label>
                                <input id="reason" type="text" name="reason" value="{{ old('reason', $reservation->reason) }}"
                                    class="w-full bg-white/5 border border-white/10 text-white rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-[#8b5cf6] transition placeholder-gray-600" placeholder="{{ __('app.reason') }}...">
                                @error('reason')<p class="text-[#d946ef] text-xs mt-1">{{ $message }}</p>@enderror
                            </div>
                        </div>

                        <div class="flex justify-between items-center mt-6">
                            <a href="{{ route('admin.reservations.index') }}" class="text-sm text-gray-400 hover:text-white transition">← {{ __('app.back') }}</a>
