<x-app-layout>
    <x-slot name="header">
        <div class="topbar-title">{{ __('app.edit_department') }}</div>
        <div class="topbar-subtitle">{{ __('app.edit_department_sub', ['name' => $department->name]) }}</div>
    </x-slot>
            ...
            <div class="dark-card rounded-3xl overflow-hidden border border-white/5">
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.departments.update', $department) }}">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label for="name" class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">{{ __('app.department_name') }}</label>
