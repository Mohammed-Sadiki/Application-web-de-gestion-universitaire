<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            <div>
                <div class="topbar-title">{{ __('app.departments') }}</div>
                <div class="topbar-subtitle">{{ __('app.departments_subtitle') }}</div>
            </div>
        ...
        <div class="dark-card rounded-3xl overflow-hidden border border-white/5">
            <div class="p-6 bg-[#17192a]/30 border-b border-white/5">
                <h3 class="text-lg font-bold text-white flex items-center gap-2">
                    ...
                    {{ __('app.departments_list') }}
                </h3>
            </div>
            <div class="p-6">
                @if($departments->isEmpty())
                    <p class="text-gray-400 italic text-center py-6">{{ __('app.no_departments') }}</p>
                @else
                    ...
                    <table class="min-w-full divide-y divide-white/5">
                        ...
                        <tbody class="text-sm divide-y divide-white/5 text-gray-300">
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors">
                                <td class="py-4 font-semibold text-white">{{ $dept->name }}</td>
                                <td class="py-4 text-right space-x-2 whitespace-nowrap">
                                    <a href="{{ route('admin.departments.edit', $dept) }}" class="inline-flex items-center px-3 py-1 bg-[#8b5cf6]/10 hover:bg-[#8b5cf6]/20 border border-[#8b5cf6]/20 text-[#8b5cf6] text-xs font-bold rounded-xl transition">{{ __('app.edit') }}</a>
