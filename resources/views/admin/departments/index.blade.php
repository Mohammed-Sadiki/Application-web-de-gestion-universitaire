<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            <div>
                <div class="topbar-title">Gestion des Filières</div>
                <div class="topbar-subtitle">Gérez les filières et départements de l'établissement</div>
            </div>
            <a href="{{ route('admin.departments.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#8b5cf6] to-[#d946ef] hover:opacity-90 text-white text-xs font-bold rounded-xl shadow-md transition duration-200">
        ...
        <div class="dark-card rounded-3xl overflow-hidden border border-white/5">
            ...
            <div class="p-6">
                @if($departments->isEmpty())
                    <p class="text-gray-400 italic text-center py-6">Aucune filière enregistrée.</p>
                @else
                    ...
                    <table class="min-w-full divide-y divide-white/5">
                        ...
                        <tbody class="text-sm divide-y divide-white/5 text-gray-300">
                            ...
                            <tr class="hover:bg-white/[0.02] transition-colors">
                                <td class="py-4 font-semibold text-white">{{ $department->name }}</td>
                                <td class="py-4 text-right space-x-2">
                                    <a href="{{ route('admin.departments.edit', $department) }}" class="inline-flex items-center px-3 py-1 bg-[#8b5cf6]/10 hover:bg-[#8b5cf6]/20 border border-[#8b5cf6]/20 text-[#8b5cf6] text-xs font-bold rounded-xl transition">Modifier</a>
