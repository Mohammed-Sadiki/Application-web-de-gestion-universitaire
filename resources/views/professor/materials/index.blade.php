<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            <div>
                <div class="topbar-title">{{ __('app.materials_title') }}</div>
                            ...
                            <tbody class="text-sm divide-y divide-white/5 text-gray-300">
                                @foreach($materials as $material)
                                    <tr class="hover:bg-white/[0.02] transition-colors">
                                        <td class="py-4 font-semibold text-white whitespace-nowrap">{{ $material->title }}</td>
                                        <td class="py-4 font-semibold text-white whitespace-nowrap">{{ $material->module->name }}</td>
                                        ...
                                        <td class="py-4 text-right whitespace-nowrap">
                                            <form method="POST" action="{{ route('professor.materials.destroy', $material) }}" onsubmit="return confirm('{{ __('app.confirm_delete') }}');" class="inline">
                                                @csrf
                                                @method('DELETE')
