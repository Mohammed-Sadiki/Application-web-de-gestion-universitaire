<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            <div>
                <div class="topbar-title">{{ __('app.upload_material') }}</div>
                <div class="topbar-subtitle">{{ __('app.materials_subtitle') }}</div>
            </div>
            ...
            <div class="dark-card rounded-3xl overflow-hidden border border-white/5">
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('professor.materials.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-5">
                            <label for="title" class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{{ __('app.material_title_lbl') }}</label>
                            <input id="title" class="w-full bg-[#0d1220] border border-white/10 text-white rounded-xl px-4 py-2.5 focus:border-[#8b5cf6] focus:ring-1 focus:ring-[#8b5cf6]" type="text" name="title" value="{{ old('title') }}" required autofocus />
                        ...
                        <div class="mb-6">
                            <label for="file" class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{{ __('app.material_file_lbl') }} (PDF, PPTX, DOCX, ZIP — Max 10MB)</label>
