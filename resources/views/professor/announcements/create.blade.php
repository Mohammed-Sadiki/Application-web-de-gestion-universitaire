<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center w-full">
            <div>
                <div class="topbar-title">{{ __('app.publish_announcement') }}</div>
            ...
            <div class="dark-card rounded-3xl overflow-hidden border border-white/5">
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('professor.announcements.store') }}">
                        ...
                        <div class="mb-6">
                            <label for="content" class="block text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{{ __('app.announcement_content_lbl') }}</label>
                            <textarea id="content" name="content" rows="6" class="w-full bg-white/5 border border-white/10 text-white rounded-xl px-4 py-2.5 focus:border-[#8b5cf6] focus:ring-1 focus:ring-[#8b5cf6]" required>{{ old('content') }}</textarea>
